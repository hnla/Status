...
jq(document).ready( function() {
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				if ( 0 == jq("ul.activity-list").length ) {
					jq(".error").slideUp(100).remove();
					jq("#message").slideUp(100).remove();
					jq(".activity").append( '<ul id="activity-stream" class="activity-list item-list">' );
				}
	...
	jq('.activity-type-tabs').click( function(event) {
	...
	jq('div.activity').click( function(event) {
		...
		if ( target.hasClass( 'spam-activity' ) ) {
			var li = target.parents( '.activity ul li' );
			target.addClass( 'loading' );
...
function bp_activity_request(scope, filter) {
	...
	function(response)
	{
		...
		jq('body .activity').fadeOut( 100, function() {
